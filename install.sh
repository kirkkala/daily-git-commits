...
echo -e "${COLOR_BLUE}║                                                       ║${COLOR_RESET}"
echo -e "${COLOR_BLUE}║           📅  daily-commits installer  ✨             ║${COLOR_RESET}"
echo -e "${COLOR_BLUE}║                                                       ║${COLOR_RESET}"
echo -e "${COLOR_BLUE}╚═══════════════════════════════════════════════════════╝${COLOR_RESET}"
echo ""
...
if [[ ! -d "$CONFIG_DIR" ]]; then
  mkdir -p "$CONFIG_DIR"
  echo -e "${COLOR_GREEN}✓ Created config directory: ${CONFIG_DIR}${COLOR_RESET}"
fi
...
if [[ -z "$SKIP_CONFIG" ]]; then
  ...
  if [[ ! -d "$PROJECTS_DIR" ]]; then
    echo -e "${COLOR_YELLOW}⚠️  Warning: Directory does not exist: ${PROJECTS_DIR}${COLOR_RESET}"
    read -p "$(echo -e ${COLOR_YELLOW})Create it now? (y/N) $(echo -e ${COLOR_RESET})" -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
      mkdir -p "$PROJECTS_DIR"
      echo -e "${COLOR_GREEN}✓ Created directory${COLOR_RESET}"
    else
      echo -e "${COLOR_BLUE}ℹ️  Continuing anyway. Make sure to create it before using daily-commits.${COLOR_RESET}"
    fi
  fi
  ...
  if ! [[ "$MAXDEPTH" =~ ^[0-9]+$ ]]; then
    echo -e "${COLOR_YELLOW}⚠️  Invalid depth, using default: 3${COLOR_RESET}"
    MAXDEPTH=3
  fi
...
EOF
  ...
  echo ""
  echo -e "${COLOR_GREEN}✓ Configuration saved to ${CONFIG_DIR}/.env${COLOR_RESET}"
fi
